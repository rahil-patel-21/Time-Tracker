...
class PlatformAlertDialogg extends PlatformWidget {
  PlatformAlertDialog(
      {@required this.title,
...
class PlatformExceptionAlert extends PlatformAlertDialog {
  ...
  static String _platformException(PlatformException platformException) {
    if (platformException.message == 'FIRFirestoreErrorDomain') {
      if (platformException.code == 'Error 7') {
        return "You Don't Have Enough Permission to Perform this Action";
      }
    }

    if (platformException.code == 'Error performing setData') {
      return "You Don't Have Enough Permission to Perform this Action";
    }

    print('Okay' + platformException.code);
